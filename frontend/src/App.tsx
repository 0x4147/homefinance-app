    ...
    useEffect(() => {
        const loadInsights = async () => {
            try {
                setLoading(true);
                setError('');
                const res = await apiService.getAthenaInsights();
                setAnalysis(res.analysis || '');
            } catch (e) {
                console.error('Failed to fetch Athena insights', e);
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-6 shadow-sm mb-8">
            ...
            <div className="bg-gradient-to-r from-purple-50 to-blue-50 border border-purple-200 rounded-lg p-6">
                {loading ? (
                    ...
                    <p className="text-gray-700 text-lg leading-relaxed whitespace-pre-line">{analysis}</p>
                )}
                <div className="mt-4 text-sm text-gray-600">
                    <p>• Analysis based on transactions from the past 3 months</p>
                    <p>• Local AI first with ChatGPT fallback</p>
                </div>
            </div>
        </div>
    );
...
const SpendingInsights = () => {
    ...
    const handleAthenaSubmit = async () => {
        if (!athenaQuery.trim()) {
        ...
        setAthenaResponse('');
        try {
            const res = await apiService.athenaChat(athenaQuery);
            setAthenaResponse(res.answer);
        } catch (e) {
            console.error('Athena chat failed', e);
            setAthenaResponse('Athena is unavailable. Please try again later.');
        } finally {
